...
const hasAismTag = (tagNames = []) =>
  tagNames.some((tag) => tag.includes(AISM_TAG));
...
exports.handler = async (event) => {
  ...
  try {
    ...
    events = events.filter((entry) => {
      ...
      const directTagNames = extractTagNames(
        eventData.tags || entry.tags || [],
      );
      ...
      return (
        hasAismTag(allTags) ||
        name.toUpperCase().includes(AISM_TAG) ||
        description.toUpperCase().includes(AISM_TAG)
      );
    });
