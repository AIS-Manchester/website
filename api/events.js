...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Filter events by AISM tag if present in the event data
    ...
    const hasAismTag = (tagNames = []) =>
      tagNames.some((tag) => tag.includes("AISM"));

    // Filter for events with AISM tag (case-insensitive)
    ...
    events = events.filter((entry) => {
      ...
      // Check if AISM appears in tags, name, or description
      return (
        hasAismTag(allTags) ||
        name.toUpperCase().includes("AISM") ||
        description.toUpperCase().includes("AISM")
      );
    });
    ...
    events.sort((a, b) => {
      const dateA = new Date(
        (a.event || a).start_at || (a.event || a).start_time,
      );
      const dateB = new Date(
        (b.event || b).start_at || (b.event || b).start_time,
      );
      return dateA - dateB;
    });
